...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .columns-row th { padding: 6px 8px; text-align: left; font-size: 8px; font-weight: bold; border-bottom: 1px solid #ccc; color: #333; }
        .columns-row th.right { text-align: right; }
        table.report-table td { padding: 5px 8px; border-bottom: 1px solid #eee; font-size: 8px; vertical-align: top; }
        table.report-table td.right { text-align: right; }
        table.report-table td.green { color: #16a34a; }
        table.report-table td.red { color: #dc2626; }
        table.report-table tbody tr:nth-child(even) { background-color: #fafafa; }
        .section-header-row td { background-color: #fff7ed; padding: 6px 10px; font-size: 10px; font-weight: bold; color: #92400e; border-left: 4px solid #d97706; border-bottom: none; }
        .subtotal-row { background-color: #fff7ed; font-weight: bold; }
        .subtotal-row td { padding: 6px 8px; border-top: 2px solid #fed7aa; font-size: 9px; }
        .spacer-row td { padding: 8px 0; border-bottom: none; }
        .grand-total-row td { background-color: #1e3a5f; color: white; padding: 8px 8px; font-size: 9px; font-weight: bold; border-bottom: none; }
            ...
            <tbody>
                @foreach ($groupedByCategory as $parentName => $group)
                    <tr class="section-header-row">
                        <td colspan="6">Categoría: {{ $group->parent_name }} - {{ $group->parent_code }}</td>
                    </tr>
                    ...
                    <tr class="subtotal-row">
                        ...
                        <td class="right green">{{ number_format($group->subtotals->entries, 2) }}</td>
                        <td class="right red">{{ number_format($group->subtotals->exits, 2) }}</td>
                        <td class="right">{{ number_format($group->subtotals->final_stock, 2) }}</td>
                    </tr>
                    <tr class="spacer-row">
                        <td colspan="6"></td>
                    </tr>
                ...
                <tr class="grand-total-row">
                    ...
                    <td class="right">{{ number_format($totals['final_stock'], 2) }}</td>
                </tr>
            </tbody>
        </table>
