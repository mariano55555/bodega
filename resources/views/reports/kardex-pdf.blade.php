...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .footer { position: fixed; bottom: -20mm; left: 0; right: 0; text-align: center; font-size: 7px; color: #666; border-top: 1px solid #ddd; padding-top: 5px; }
        ...
        .info-cell { width: 25%; text-align: right; font-size: 9px; }
        .institution-name { font-size: 12px; font-weight: bold; color: #1e3a5f; margin-bottom: 3px; }
        .department-name { font-size: 10px; color: #555; margin-bottom: 5px; }
        .report-title { font-size: 13px; font-weight: bold; color: #1e3a5f; margin-bottom: 3px; }
        .header-separator { border-bottom: 2px solid #1e3a5f; margin-bottom: 3px; }
        .info-section { background-color: #f5f5f5; padding: 5px 10px; }
        .info-table { width: 100%; border-collapse: collapse; }
        .info-table td { padding: 3px 5px; font-size: 9px; }
        .info-table .label { font-weight: bold; color: #555; width: 20%; }
        .info-table .value { color: #333; width: 80%; }
        table.data-table { width: 95%; border-collapse: collapse; margin-bottom: 15px; margin-left: auto; margin-right: auto; }
        table.data-table thead tr.columns-row { background-color: #e8e8e8; }
        table.data-table th { padding: 6px 4px; text-align: left; font-size: 8px; font-weight: bold; border-bottom: 1px solid #ccc; color: #333; text-transform: uppercase; }
        table.data-table th.right { text-align: right; }
        table.data-table td { padding: 5px 4px; border-bottom: 1px solid #eee; font-size: 8px; }
        ...
        .summary { margin-top: 15px; background-color: #1e3a5f; color: white; padding: 12px 15px; width: 95%; margin-left: auto; margin-right: auto; }
        .summary-table { width: 100%; border-collapse: collapse; }
        .summary-table td { padding: 4px 10px; font-size: 10px; }
        .summary-table .label { font-weight: bold; text-align: right; width: 70%; }
        .summary-table .value { text-align: right; width: 30%; font-weight: bold; }
        .summary-table .separator td { border-top: 1px solid rgba(255, 255, 255, 0.3); padding-top: 8px; margin-top: 4px; }
...
<body>
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="info-cell"><div>Fecha de reporte:</div><div>{{ now()->format('d/m/Y') }}</div></td>
        </tr>
    </table>
    ...
    <div class="summary">
        <table class="summary-table">
            <tr><td class="label">Total Entradas:</td><td class="value">{{ number_format($totalIn, 2) }}</td></tr>
            <tr><td class="label">Total Salidas:</td><td class="value">{{ number_format($totalOut, 2) }}</td></tr>
            <tr><td class="label">Valor en Inventario:</td><td class="value">${{ number_format($finalValue, 2) }}</td></tr>
            <tr class="separator"><td class="label">Total Movimientos:</td><td class="value">{{ $movements->count() }}</td></tr>
            <tr><td class="label">Existencia Actual:</td><td class="value">{{ number_format($lastMovement->balance_quantity, 2) }}</td></tr>
            <tr><td class="label">Costo Unitario Actual:</td><td class="value">${{ number_format($lastMovement->unit_cost ?? 0, 2) }}</td></tr>
        </table>
    </div>
