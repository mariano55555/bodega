...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .columns-row th { padding: 5px 6px; text-align: left; font-size: 7px; font-weight: bold; border: none; color: white; }
        ...
        .parent-header-row td { background-color: #1e3a5f; color: white; padding: 6px 10px; font-size: 9px; font-weight: bold; border: 1px solid #1e3a5f; }
        .section-header-row td { background-color: #3b6998; color: white; padding: 5px 10px; font-size: 8.5px; font-weight: bold; border: 1px solid #3b6998; }
        .subtotal-row { background-color: #e8e8e8; font-weight: bold; }
        .subtotal-row td { padding: 5px 6px; border-top: 1px solid #999; font-size: 7.5px; }
        .parent-subtotal-row { background-color: #d0d0d0; font-weight: bold; }
        .parent-subtotal-row td { padding: 6px; border-top: 2px solid #999; font-size: 8px; }
        .spacer-row td { padding: 4px 0; border: none; }
        .small-spacer-row td { padding: 2px 0; border: none; }
            ...
            <tbody>
                @foreach ($groupedByCategory as $group)
                    ...
                    <tr class="parent-header-row">
                        <td colspan="8">{{ $group->parent_name }} — Código {{ $group->parent_code }}</td>
                    </tr>
                    @foreach ($group->subcategories as $subcategory)
                        {{-- Subcategory / Línea Presupuestaria header --}}
                        <tr class="section-header-row">
                            <td colspan="5">Línea Presupuestaria: {{ $subcategory->category_name }}</td>
                            <td colspan="3" style="text-align: right;">Específico {{ $subcategory->category_code }}</td>
                        </tr>
                        ...
                        <tr class="subtotal-row">
                            <td colspan="2" style="text-align: right;">Total Línea {{ $subcategory->category_code }}</td>
                            <td class="right">{{ number_format($subcategory->subtotals->initial_stock, 2) }}</td>
                            <td class="right">{{ number_format($subcategory->subtotals->entries, 2) }}</td>
                            <td class="right">{{ number_format($subcategory->subtotals->exits, 2) }}</td>
                            <td class="right">{{ number_format($subcategory->subtotals->current_stock, 2) }}</td>
                            <td></td>
                            <td class="right">${{ number_format($subcategory->subtotals->total_cost, 2) }}</td>
                        </tr>
                    ...
                    <tr class="parent-subtotal-row">
                        <td colspan="2" style="text-align: right;">Total {{ $group->parent_name }}</td>
                        <td class="right">{{ number_format($group->parent_subtotals->initial_stock, 2) }}</td>
                        <td class="right">{{ number_format($group->parent_subtotals->entries, 2) }}</td>
                        <td class="right">{{ number_format($group->parent_subtotals->exits, 2) }}</td>
                        <td class="right">{{ number_format($group->parent_subtotals->current_stock, 2) }}</td>
                        <td></td>
                        <td class="right">${{ number_format($group->parent_subtotals->total_cost, 2) }}</td>
                    </tr>
                    <tr class="spacer-row">
                        <td colspan="8"></td>
                    </tr>
